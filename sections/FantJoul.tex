...
First, to refine the static component of the model it is separated in $P_{\textnormal{base}}$, the power draw when the device is on without, and $P_{\textnormal{interface}}$, the power draw per interface with respect to its configuration which should be summed for each interface. 
...
To this end, the experimental setup is composed of 3  components :
...
\begin{description}
	...
	\item [\textbf{PortActive}] \hspace{18pt} Ports are activated, but interfaces remains down. 
...
Deriving the 7 power model parameters from the measured power of a scenario $P_\textnormal{<scenario>}$ can be down using the following equations :
...
\end{equation}
\begin{equation}
	P_{\textnormal{\textbf{Snake}}} =  P_{\textnormal{\textbf{Trx}}} + P_\textnormal{dyn}
	 \label{snake}
\end{equation}
...
Finally, to derive the energy per bits ($E_\textnormal{bit}$) and packets ($E_\textnormal{bit}$), the approach proposed by the earlier work \cite{35} is followed. 
The total packet size is $S_\text{tot} = S + S_h$ with $S_h$ the header size and $S$ the size of the packet body.
The model assumes that, for a fixed body size $S$, the power is a linear function of the bit rate $r$, knowing $S_h$ is constant.
Since, the packet rate is constant, it is an intuitive and practical assumption, as it permits to distinguish $E_\textnormal{bit}$ from $E_\textnormal{bit}$ by varying $S$ and deduce its effect on the power which is only correlated to $E_\textnormal{bit}$ and $r$. 
...
Moreover, since packets have a controlled size $S$ in bytes, the bit and packet rate ($p$ and $r$) are related by $p = r/8 \cdot S_\text{tot}$. Hence, Eq (\ref{energy}) rewrites: 
\begin{equation}
...
Since, the size $S$ is controlled, we have that Eq. (\ref{impl1}) is the same as to Eq. (\ref{impl2}), hence 
...
Repeating this process for multiples values of $S$ gives a set of couples ($S_i$, $\alpha_{Si}$) which allows computing multiples values of the right-hand side of Eq (\ref{eq:final}) in relations to the corresponding $S$ from Eq (\ref{eq:final}).
This process results in a set of packet body size coupled $S$ with their total corresponding energy consumption (left-hand side of Eq (\ref{eq:final})).
Second, from the latter computed set, linear regression of the energy is once again used over $S$.
The resulting intercept represents $E_\textnormal{bit} \cdot 8S_h + E_\textnormal{pkt}$. 
The resulting slope gives $E_\textnormal{pkt} \cdot 8$. 
...
\end{equation}
This intercept is used as it logically perfectly aligns with the linear function and is already computed. 
