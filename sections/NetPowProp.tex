...
\section{It Is Time to Address Network Power Proportionality} \label{sec:NetPowProp}
...
However, it is observed  (Section \ref{sec:impact}) that, mainly due to the combination of poor power proportionality and low time usage, the network accounts for an average of 12\% of the total power footprint in the modeled cluster.
...
\begin{tikzpicture}[
    % --- THE GLOBAL GRID SPACING ---
    % First value = Vertical gap between layers
    % Second value = Horizontal gap between sibling nodes
    % REDUCE the second value (e.g., from 0.4cm to 0.1cm) to narrow the figure globally!
    ...
    % --- LAYER 1 ---
    % We use an explicit xshift on this layer to set the initial primary branch width
    \node[node style, below left=of R, xshift=-0.8cm] (L1-1) {};
    \node[node style, below right=of R, xshift=0.8cm] (L1-2) {};

    % --- LAYER 2 ---
    \node[node style, below left=of L1-1] (L2-1) {};
    \node[node style, below right=of L1-1] (L2-2) {};
    
    \node[node style, below left=of L1-2] (L2-3) {};
    \node[node style, below right=of L1-2] (L2-4) {};

    % --- LAYER 3 (Leaves) ---
    \node[node style, below left=of L2-1] (L3-1) {};
    \node[node style, right=of L3-1] (L3-2) {};
    
    \node[node style, right=of L3-2] (L3-3) {};
    \node[node style, right=of L3-3] (L3-4) {};

    \node[node style, below right=of L2-4] (L3-8) {};
    \node[node style, left=of L3-8] (L3-7) {};
    
    \node[node style, left=of L3-7] (L3-6) {};
    \node[node style, left=of L3-6] (L3-5) {};
    

    % --- EDGES ---
    % Root to Layer 1
    \draw[edge style, line width=6pt] (R) -- (L1-1);
    \draw[edge style, line width=6pt] (R) -- (L1-2);

    % Layer 1 to Layer 2
    \draw[edge style, line width=3.5pt] (L1-1) -- (L2-1);
    \draw[edge style, line width=3.5pt] (L1-1) -- (L2-2);
    \draw[edge style, line width=3.5pt] (L1-2) -- (L2-3);
    \draw[edge style, line width=3.5pt] (L1-2) -- (L2-4);

    % Layer 2 to Layer 3
    \draw[edge style, line width=1.8pt] (L2-1) -- (L3-1);
    \draw[edge style, line width=1.8pt] (L2-1) -- (L3-2);
    \draw[edge style, line width=1.8pt] (L2-2) -- (L3-3);
    \draw[edge style, line width=1.8pt] (L2-2) -- (L3-4);
    \draw[edge style, line width=1.8pt] (L2-3) -- (L3-5);
    \draw[edge style, line width=1.8pt] (L2-3) -- (L3-6);
    \draw[edge style, line width=1.8pt] (L2-4) -- (L3-7);
    \draw[edge style, line width=1.8pt] (L2-4) -- (L3-8);

\end{tikzpicture}
	\end{center}
	\caption{Fat tree topology example, the thicker links represent larger bandwidth.}\label{fig:fatTree}
\end{figure}
...
\begin{itemize}
	...
	\item a fat tree topology, which is a network topology design exemplified in Figure \ref{fig:fatTree}, using \qty{51.2}{\tera\bit\s} switches
\end{itemize}
...
This model is practical yet oversimplified as it disregards complex behaviors to ease calculations. 
...
Although the actual behavior might be non-linear, the model assumes linearity between \textit{idle} and \textit{max} power to provide a simple and intuitive definition.  
...
The power draw of NICs and transceivers with respect to the considered bandwidth is featured in Table \ref{tab:PowNicTrans}.
Missing values are extrapolated by considering the that the same scale $b$ when doubling the bandwidth from the last available values $(v_1, v_2)$ such that $v_1 < v_2$ and $b= v_2/v_1$.  
Hence, the extr.apolated $e$ value can be calculated from the last available value $v$ by computing $e = v*b$.
Appendix \ref{app:routercompo} shows a schema of the inner components of a router.
Again, as for the workload model, the simple extrapolation underestimates by overlooking overheads due to scaling.     
...
\begin{table}[t]
	...
	\centering
		\begin{tabular}[c]{cccccc}
			...
			\bottomrule
		\end{tabular}
		\caption{\hspace{4.5cm} * extrapolated value}
\end{table}

To be able to calculate the overall power drawn of the cluster, it is necessary to know the number of switches, NICs, and transceivers.
...
Following the release of the GitHub repository containing all calculations for the figures, the underlying computation of the number of switches became clear.
...
Then, the number of switches $N_\textnormal{Sw}$ is computed using the number of layers $L$ :
\begin{equation}
L = \frac{log(N_\textnormal{hosts}/2)}{log(N_\textnormal{port}/2)}
\end{equation}
\begin{equation}
N_\textnormal{Sw} = (2L - 1) \cdot (N_\textnormal{port}/2)^{L-1}
\end{equation}
...
With the computed number of components and their power values, the power footprint of the networking and computing hardware for the examined ML cluster calculated by the article depending on the phase is presented in Figure \ref{fig:PowFootprint}.   
...
Those results are significant, as even 5\% of reduction, due to the scale of the cluster, is a considerable amount of power.


\begin{figure}[t]
		\centering
		\includegraphics[width=0.95\columnwidth]{figures/PowerFootPrintCompexp.png}
	\caption{Absolute power footprint of an iteration and its efficiency.}\label{fig:PowAbs}
\end{figure}
...
In this section, we focus on the impact of low proportionality of state-of-the-art networking hardware in the context of a machine learning cluster. 
Modeling the use of one pod of this cluster (Section \ref{sec:PowMod}) was initially complicated due to the lack of transparency regarding topology interpolation, but was solved by the release of their GitHub repository \cite{githubeNetPowProp}.
...
In the context of green networking, such amounts represent non-negligible insights and should be a concern to investigate further, especially in the case of machine learning clusters or data centers, due to their recent growth in number, large scale and low communication rate.  

To investigate possible solutions, improvements regarding the power proportionality of network devices are explored from static and dynamic viewpoints by taking inspiration from the computing hardware strategies.
