...
Ensuring the correctness of network configurations for specific properties (\textit{e.g.}, reachability) in a scalable and precise manner is an important problem of interest to ISPs and large companies.
This problem, due to the scale of the deployed networks in such facilities and complex interdependencies between routing protocols, goes beyond the capabilities of current network analysers, such as Batfish \cite{batfish} or Minesweeper \cite{Minesweeper}, as they would have to determine all scenarios for which the property is violated.  
...
To address this problem, we explore the reasoning and the implementation behind NetDice \cite{steffen2020netdice}, a probabilistic verification tool for network configuration published as a full conference paper at ACM SIGCOMM 2020. 
It supports modern routing protocols such as BGP and OSPF, and features such as ECMP and static routes.
...
Examples of properties and their meaning are shown in Table \ref{tab:props}. 
...
Finally, the results of NetDice's usage on synthetic configurations and a nation-level ISP are presented (Section \ref{sec:results}).

\subsection{Cold Edges}\label{sec:HotEdges}

This section aims to introduce the concept of cold edges.
Their definition and computation using an internal routing protocol (OSPF) considering static routes and ECMP is initially presented.
Then BGP is incorporated to the model to allow NetDice to reason with external peers.  

As equal-cost multiple path routing (ECMP) is a supported feature, flows in a network can take multiple paths if they share the same cost.
To represent such behaviour in the configuration graph $G~=~(V,E)$, the internal routing protocol gives a forwarding graph $G_\textnormal{fwd} \subseteq G$, composed of a set of vertices (routers) $V_\textnormal{fwd} \subseteq V$  and a set of directed edges (links) $E_\textnormal{fwd} \subseteq E$.
...
Multiple link failures form a failure scenario $l \subseteq E$, which is a set containing the unusable links.
...
Defining if a property holds in a certain configuration scenario strictly depends on the forwarding graphs, as properties are defined for flows and verified for the internal paths used to connect both ends of flows. 
...
\textbf{For the internal routing scenario}, an algorithm to compute hot edges considering the shortest paths and  static routes is presented in Alg. \ref{alg:cold}.
It takes a flow $(u,d)$,  the set of static routes $Static_d \subseteq V\times V$ towards the destination $d$, a forwarding graph's edges $E_\textnormal{fwd}$ considering the static routes, and a failure scenario $l$ as parameters. 
For each static routes $(x, y)$ contained in $Static_d$, the source $x$ uses the next-hop $y$ to route traffic going towards $d$.      
This can lead, as discussed below, to the rerouting of the initial shortest paths considered by any traversed node, especially the source node of the flow $u$.
...
To compute the edges along the shortest path we define two side algorithms : 
\begin{definition}$\textnormal{SP}_l(s,t)$
	returns the set of all edges along the shortest paths from $s$ to $t$, under link states defined by $l$.
\end{definition}
...
Algorithm \ref{alg:cold} \textsc{HotSpStatic} starts by marking as hot every edge along the shortest paths from the source $u$ to the destination $d$ under the failure scenario $l$ (Lin. 2).
...
Then, all the shortest paths from the end nodes of traversed static routes are marked hot (Lin.3-4), as just like the source $u$, they consider their shortest paths towards the destination $d$. 
...
As an example, Figure \ref{fig:cold} represents the application of Alg \ref{alg:cold} giving the hot and cold edges of a certain network configuration in which : every link is functional ($l = \emptyset $), the flow is (D, C), the forwarding edges are $E_\textnormal{fwd} = \{(D,E),(E,B),(B,C)\}$, and the static routes towards $C$ are $Static_C = \{(E,B)\}$.
Note that the $(E,C)$ edge is marked as hot even though it is not in the forwarding graph, because it lies on the shortest path of D, which is $D\rightarrow E\rightarrow C$ with cost 2.
If it were to fail, the decision of D would be to use $D\rightarrow A\rightarrow B\rightarrow C$ of cost 4 as its shortest path, changing the forwarding graph.  
...
\begin{figure}
	\begin{center}
		\includesvg[width=0.95\columnwidth]{figures/exampleBGP.drawio.svg}
	\end{center}
	\caption{Example of the BGP forwarding. The router $u$ selected $a$ as its next-hop towards the external destination $d$ traversing $c$ on the way.}\label{fig:bgpExampl}
\end{figure}


The Border Gateway Protocol (BGP) is the \textit{de facto} standard Internet routing protocol and is used throughout the world to route IP packets.   
It connects large independent networks known as Autonomous Systems (AS) and forwards packets by considering paths of theses ASes.    
For a certain destination $d$, a router $u$ has to choose which BGP route it will use to select a next-hop to forward the packet to.   
This decision process is based on path attributes and is described in Table \ref{tab:bgprocess}, Figure \ref{fig:bgpExampl} illustrate an example of BGP forwarding.
...
Considering those new possibilities for a forwarding graph to change, the algorithm to compute the hot edges has to assess two properties. (i) keeping the connectivity between BGP peers and (ii) the IGP cost towards a selected next-hop stays unchanged.
...
\begin{definition}$\textnormal{Top3}(B_R,X)$\label{alg:top3}
	returns, given the subset of nodes $X$ and the border routers $B_R$, the subset of border routers $B_{Rl} \subseteq B_R$ announcing the highest preference routes globally.   
\end{definition}
...
\begin{figure*}[t]
	...
	\caption{Computation of hot edges in a BGP capable network configuration.  The flow is $(B,d)$ with d the external BGP destination advertised by O, P and Q, LP represents the local preference of the announcements towards d. The border routers towards $d$ are $B_R = \{E,A,C\}$, and the route reflectors $R_R = \{V,W\}$ represented in purple, the selected next-hop is represented in green at each node. Hot edges are highlighted in red.}\label{fig:hotbgp}
\end{figure*}
An example of the application of Algorithm \ref{alg.hotbgp} can be found in Figure \ref{fig:hotbgp}. The route reflectors are V and W with respectively \{E, D, C, W\} and \{F, B, V\} as their clients. The Border routers are E, A and C. There are no static routes and the flow is (B, $d$), with $d$ the external destination advertised by O, P and Q.
After the BGP simulation, the forwarding graph is $B \rightarrow C \rightarrow V \rightarrow E$.
Notice that the B packets leave the network at E and not at A which is B's selected next-hop. This is due to B's shortest path to A ($B \rightarrow C \rightarrow F \rightarrow W \rightarrow A$) which traverses C whose next-hop is E, rerouting B's origin forwarding plan to leave the network at E, sometimes called a deflection.
...
Then, Figure \ref{fig:hotbgp}(iib) represents the union of the added hot edges from line 6 to 10. It ensures that every edges that could make the IGP cost towards $B_Rl$ change is marked as hot.
The shortest path from B towards its selected next-hop is added in line 6 to ensure that IGP cost towards A stays unchanged. This adds ${(B,C), (C,F), (F,W), (W,A)}$ to the set of hot edges.
...
The exploration of the failure space is achieved by only considering the possible failures of hot edges. 
...
\begin{enumerate}
  ...
  \item compute the hot edges with the forwarding graphs using Alg. \ref{alg.hotbgp}
...
An example of execution on the network configuration from Figure \ref{fig:cold} with property $\phi =$ "Traffic from D to C traverses E" ($Waypoint_{D\rightarrow C}(E)$) is shown in Figure \ref{fig:execTree}.
...
\begin{enumerate}
  ...
  \item A probability mass function of link failure $P(L)$, such that, for a failure scenario represented by a vector $l$ assigning every link to up or down, its probability is defined by $P(L = l)$.
\end{enumerate}

A simple distribution assigning to each link $e \in E$ a probability to fail independently resulting in $P(L = l) = \prod_{e \in E} P(L_e = l_e)$ is considered in Figure \ref{fig:execTree} with the probability of a link to fail set to $P(L_e = 0)=p=0.1$, conversely $P(L_e = 1)=\tilde{p}=0.9$.
...
Similarly, the failure of an SRLG is modeled by the failure of the links in the group. 
...
The following Section presents NetDice's algorithm to compute the probability of a property given a network configuration and a probability distribution. 
...
Four functions are used by the algorithm.

\begin{definition}{$\textnormal{flow}(\phi)$} returns the set of flows of the property $\phi$ 
\end{definition}
\begin{definition}{$\textnormal{Scenario}(s, T)$} returns the corresponding failure scenario of the state $s \subseteq \{0,1,?\}^{|E|}$ by setting every undefined link in the subset $T \subseteq E$ to be up.
\end{definition}
...
This is done to consider hot edges as up and disregard the undefined cold links, as regardless of their state, they do not have impact on whether $\phi$ holds or not.
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    \Procedure{Explore}{s}
		...
		\For{each flow $(u_i, d_i) \in \textnormal{flows}(\phi)$}  
    ...
    \For{$e^? \in \mathcal{H}^?$}  
		...
		\State $\rho \leftarrow \rho + \textsc{Explore}(s')$ 
...
\textbf{Synthetic tests} of NetDice's implementation are done on 90 topologies, from the Topology Zoo \cite{topologyZoo} and public tier 1 transit topologies \cite{tier1Topo}, containing between 19 (resp. 50) and 754 (resp. 2 320) nodes (resp. links). 
The weight of every link is set to 1.
The probability of node (resp. link) failure used is 0.0001 (resp. 0.001).
Regarding BGP configurations, as they are not present in the original topologies, synthetic BGP setups with 2 route reflectors and 10 border routers with 2 external peers each is created by selecting random routers to take these roles. 
...
A key insight on the runtime measurements is that it is dominated by the Variable Elimination (VE) (Lin. 8 of Alg. \ref{alg:finalImpl}), as removing node failures, hence avoiding the need for complex VE usage, yields a 5 to 10 factor improvement. 
...
The manner to retrieve such cold edges is presented considering IGP and BGP protocols.   
