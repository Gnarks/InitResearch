...
Furthermore, enforcing a property at all time is not often necessary, as the concern is rather on the probability or the fraction of time of the property to hold.
...
Such properties typically aim at representing SLA's and can be either expressed as single-flow or multiple-flow considering a flow to be a pair of source and destination IP address.
...
To represent such behaviour, a forwarding graph, composed of $V_\textnormal{fwd}$ a set of vertices (routers) and $E_\textnormal{fwd}$ a set of directed edges (links), is used.
...
Defining if a property holds in a certain configuration scenario strictly depends on forwarding graphs, as properties are defined for flows and verified for the internal path used to connect both ends of flows. 
Leveraging the fact that a property holds only changes if the forwarding graphs change, NetDice's authors proposes the concept of cold and hot edges in Definition \ref{def:cold}.
...
In practice (Sec. \ref{sec:explo}), hot edges are used to prune the exploration of the possible failures scenarios.
...
Cold edges exploration are disregarded for a specific scenario, it is possible for a cold edge in a scenario to be hot in another one (See Fig. \ref{fig:execTree} (D, A) in (16) and (6)).  
...
This can lead, as discussed beneath, to the rerouting of the initial shortest path considered by any traversed node, especially the source node of the flow $u$.
...
As an example, Figure \ref{fig:cold} represents the application of Alg \ref{alg:cold} giving the hot and cold edges of a certain network configuration considering every link as functional and a flow (D, C), the forwarding edges $E_\textnormal{fwd} = \{(D,E),(E,B),(B,C)\}$ and the static routes towards $C$ $Static_C = \{(E,B)\}$.
...
\begin{definition}$\textnormal{Top3}(B_R,X)$\label{alg:top3}
	returns, given subset of node $X$ and the border routers $B_R$, the subset of border routers $B_{Rl} \subseteq B_R$ announcing the highest preference routes globally.   
\end{definition}
...
Its effect can be observed in Figure \ref{fig:hotbgp} (i) at the node C : it was filtered out of considered the border routers because its Local Preference (LP) is lower than A and O's announcements.  
...
The shortest path from B towards its selected next-hop is added in line 6 to ensure unchanged in IGP costs towards A. This adds ${(B,C), (C,F), (F,W), (W,A)}$ to the set of hot edges.
...
It is assumed that a \textit{check} function returning, given a property and forwarding graphs, if a property holds in every forwarding graph is implemented for each property.
...
\todo[inline]{relire tout et faire la conclu}
