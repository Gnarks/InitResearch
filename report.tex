...
But the key insight of NetDice is to prune the exploration of failure scenarios where certain the state of certain links is guaranteed to have no impact on the state of $\phi$. 
...
\begin{enumerate}
  ...
  \item $\forall h \in \mathcal{H}^?$ make two recursive calls (going back to \textit{pt}1) : with $h$ up and $h$ down 
...
Recall that multiple forwarding graphs are possibly computed as equal-cost multi path (ECMP) is considered.
